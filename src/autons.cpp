...
void skills()
{
    ...
    chassis.moveToPoint(2, 55, 1100, {.maxSpeed = 90,.minSpeed=10});
    chassis.turnToHeading(0, 600);
    chassis.moveToPoint(2, 71, 800, {.maxSpeed = 80,.minSpeed=10});
    chassis.waitUntil(5);
    fastintake.move_relative(-100, 600);
    arm.move_absolute(550, 200);
    chassis.cancelAllMotions();
    chassis.moveToPoint(2, chassis.getPose().y - 2, 500, {.forwards = false,.minSpeed=10});
    chassis.turnToHeading(0, 300);
    chassis.moveToPoint(2, 70, 500, {.maxSpeed = 80,.minSpeed=10});
    ...
    chassis.turnToHeading(120, 600, {.maxSpeed = 55});
    chassis.moveToPoint(-60, 60, 900, {.forwards = false,.minSpeed=10});
    chassis.waitUntilDone();
    clamp.toggle();
    ...
    chassis.moveToPoint(-2, chassis.getPose().y + 2, 500, {.forwards = false,.minSpeed=10});
    chassis.turnToHeading(180, 300);
    chassis.moveToPoint(-2, -70, 800, {.maxSpeed = 80,.minSpeed=10});
    arm.move_relative(100, 200);
    fastintake.move_voltage(10000);
    chassis.waitUntilDone();
    ...
    delay(150);
    chassis.moveToPoint(25, -52, 1600, {.minSpeed=10});
    arm.move_absolute(80, 200);
    ...
    delay(50);
    chassis.turnToPoint(48, 0, 800, {.maxSpeed = 90,.minSpeed=10});
    fastintake.move_voltage(12000);

    chassis.moveToPoint(32, -22, 1000,{.minSpeed=20});
    chassis.turnToPoint(24, 48, 900, {.maxSpeed = 60});
    chassis.moveToPoint(45, 8, 1400, {.forwards = false, .maxSpeed = 80});

    chassis.waitUntilDone();
    clamp.toggle();
    delay(50);
    chassis.swingToHeading(120,DriveSide::LEFT,900);
    chassis.moveToPoint(69, 0, 1000, {.maxSpeed = 70,.minSpeed=10});
    chassis.turnToHeading(90, 800, {.maxSpeed = 80});

    chassis.waitUntilDone();
    delay(50);
    chassis.setPose(63, 0, chassis.getPose().theta);
    chassis.moveToPoint(57, 0, 800, {.forwards = false});
    ...
    delay(500);
    chassis.moveToPoint(62, 48, 1300);
    chassis.turnToHeading(135, 800, {.maxSpeed = 70,.minSpeed=10});
    ...
    chassis.moveToPoint(60,-3, 1000,{ .minSpeed=60, .earlyExitRange=4});
    
    chassis.moveToPoint(67,-63,3000,{.maxSpeed=110});
    chassis.waitUntilDone();
    //chassis.moveToPoint(20, -20, 3000, {.forwards = false, .maxSpeed = 90});
    /**/
}
