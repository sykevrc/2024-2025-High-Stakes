...
void lemlib::update() {
    ...
    distRaw = dist.get_distance()+5.75;
    ...
    if (!(abs(odomPose.x)<36&&abs(odomPose.y<36))&&distRaw<1000){
        ...
        if (odomPose.x>36 && d_angle>70 && d_angle<110){
            ...
            odomPose.x = -71.5-sin(degToRad(d_angle))*distRaw;
        }else if(odomPose.y<-36 && d_angle>160 && d_angle<200){
            odomPose.y = -71.5-cos(degToRad(d_angle))*distRaw;
        }else if(odomPose.y>36 && d_angle>340 && d_angle<20){
            odomPose.y = 71.5-cos(degToRad(d_angle))*distRaw;
        }
        // float estimate = cos(fmod(fmod(avgHeading-90, 360)+360, 360))*distRaw;
        // if(abs(estimate+odomPose.x)>70 && abs(estimate+odomPose.x)>abs(estimate+odomPose.y)){
